...
class RocioHermanoImportWizard(models.TransientModel):
    ...
    file = fields.Binary(string='Archivo Excel', required=True, help='Archivo Excel con los datos de los hermanos')
    ...
    def _get_country_id(self, name):
        ...
        country = self.env['res.country'].search([('code', '=', name_str.upper())], limit=1)
    ...
    def action_import(self):
        ...
        if missing_columns:
            raise UserError(_('El archivo Excel no tiene las columnas necesarias: %s') % ', '.join(missing_columns))
        ...
        for index, row in df.iterrows():
            try:
                ...
                if pd.isna(contact_name):
                    error_count += 1
                    log_lines.append(_('Fila %s: No se especificó un nombre') % (index + 2))
                    continue

                # Obtener país primero
                country_id = self._get_country_id(row.get("País"))

                vals = {
                    "name": contact_name,
                    "street": row.get("Street"),
                    "zip": self._to_str(row.get("C.P.")),
                    "city": row.get("Ciudad"),
                    "phone": self._to_str(row.get("TELEFONO")),
                    "email": self._to_str(row.get("EMAIL")),

                    "is_brother": True,
                    "brother_district": self._to_str(row.get("DIST")),
                    "brother_birth_date": self._to_date(row.get("brother_birth_date")),
                    "brother_since": self._to_date(row.get("F ALTA")),
                    "brother_end_date": self._to_date(row.get("F BAJA")),
                    "brother_leave_reason": row.get("Motivo"),
                    "brother_advertising": self._to_bool(row.get("PUBLI")),
                    "brother_method_of_payment": self._map_payment(row.get("F DE PAGO")),
                    "brother_delegated_partner_id": self._get_partner_id_by_name(row.get("DIRECCION DE COBRO")),

                    "country_id": country_id,
                    "state_id": self._get_state_id(country_id, row.get("State_id")),
                }

                # Buscar si el contacto ya existe por nombre
                existing_partner = self.env['res.partner'].search([('name', '=', contact_name)], limit=1)
                ...
                if "Banco" in row and not pd.isna(row.get("Banco")):
                    self._create_or_update_bank_account(partner_id, row.get("Banco"))
                    log_lines.append(_('  → Cuenta bancaria asignada: %s') % row.get("Banco"))

            except Exception as e:
                error_count += 1
                log_lines.append(_('Fila %s - Error: %s') % (index + 2, str(e)))
